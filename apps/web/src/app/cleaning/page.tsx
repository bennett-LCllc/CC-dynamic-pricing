...
import type { CleaningJobSummary, Property } from '@cc-ops/shared';
import CleaningJobForm from '@/components/cleaning/CleaningJobForm';
import {
  Sparkles, Plus, Search, MapPin, User,
...
const STATUS_GROUPS = [
  ...
  { key: 'QUALITY_CHECK', label: 'Quality Check', color: 'bg-purple-100 text-purple-700' },
];

export default function CleaningPage() {
  const router = useRouter();

  const [jobs, setJobs] = useState<CleaningJobSummary[]>([]);
  ...
  const fetchJobs = async () => {
    ...
    try {
      ...
      const [jobData, propData] = await Promise.all([
        getCleaningJobs(filters),
        getProperties(),
      ]);
      setJobs(jobData);
      setProperties(propData as unknown as Property[]);
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Failed to load cleaning jobs');
    } finally {
      setLoading(false);
    }
  };
  ...
  const filteredJobs = useMemo(() => {
    ...
    return jobs.filter((j) => {
      ...
      const cleanerName = j.cleaner?.name?.toLowerCase() ?? '';
      return (
        j.cleaningType.toLowerCase().includes(q) ||
        propName.includes(q) ||
  ...
  const groupedJobs = useMemo(() => {
    const groups: Record<string, CleaningJobSummary[]> = {};
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-0.5">
              Turnover cleaning and deep cleaning management
            </p>
          </div>
          <button
      ...
      <div className="p-8 space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div className="stat-card">
            ...
            <div className="stat-value text-ocean-600">{filteredJobs.length}</div>
          </div>
          <div className="stat-card">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="relative">
            ...
            <select
              ...
              className="pl-10 pr-8 py-2 border border-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-ocean-500 appearance-none bg-white"
            >
        ...
        {loading ? (
          ...
          <div className="card p-12 text-center">
            ...
            <h3 className="text-lg font-semibold mb-2">No cleaning jobs</h3>
            <p className="text-sm text-muted-foreground max-w-md mx-auto mb-4">
              Get started by creating a new cleaning job.
            </p>
            <button
              onClick={() => setShowAddForm(true)}
              className="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium bg-ocean-600 text-white rounded-lg hover:bg-ocean-700 transition-colors"
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {groupJobs.map((job) => (
                      <JobCard
                        ...
                        onClick={() => router.push(`/cleaning/${job.id}`)}
                      />
                    ))}
                  </div>
                </div>
